...
#if defined TRACY_HW_TIMER && __ARM_ARCH >= 6 && !defined __SWITCH__
int64_t (*GetTimeImpl)();
...
void Profiler::Worker()
{
    ...
    for(;;)
    {
        ...
        for(;;)
        {
            ...
            #ifdef __SWITCH__
            while( !mutexTryLock(&m_serialLock) )
            #else
...
#  endif
    ...
    #ifdef __SWITCH__
        timespec ts;
        ts.tv_nsec = 200000000; // 200ms in nanoseconds
        timespec rem;
        thrd_sleep(&ts, &rem);
    #else
